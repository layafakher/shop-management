...
columns=('Sl No', 'اسم', 'نوع', 'مقدار باقیمانده', 'هزینه', 'موثر برای', 'تاریخ انقضا', 'مکان قفسه', 'ساخت')

def open_win(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    global apt, flag
    flag='apt'
    apt=Tk()
    apt.title("Kosar Najafi 99221001111050")
    Label(apt, text="فروشگاه دارو").grid(row=0,column=0,columnspan=3)
    Label(apt, text='*'*80).grid(row=1,column=0,columnspan=3)
    Label(apt, text='-'*80).grid(row=3,column=0,columnspan=3) 

    Label(apt, text="ذخیره انبار").grid(row=2,column=0)
    Button(apt,text='مشتری جدید', width=25, command=val_cus).grid(row=4,column=0)
    Button(apt,text='افزودن محصول به انبار', width=25,command=stock).grid(row=5,column=0)
    Button(apt,text='حذف محصول از انبار', width=25,command=delete_stock).grid(row=6,column=0)
    

    Label(apt, text="دسترسی به دیتابیس").grid(row=2,column=1)
    Button(apt,text='تغییر',width=15, command=modify).grid(row=4,column=1)
    Button(apt,text='جستجو', width=15, command=search).grid(row=5,column=1)
    Button(apt,text='بررسی انقضا', width=15, command=exp_date).grid(row=6,column=1)

    Label(apt, text="مدیریت جریان های مالی").grid(row=2,column=2)
    Button(apt,text="بررسی درآمد امروز", width=20,command=show_rev).grid(row=5,column=2)
    Button(apt,text='صورتحساب', width=20, command=billing).grid(row=4,column=2)

    Label(apt, text='-'*80).grid(row=12,column=0,columnspan=3)    
    Button(apt,text='خروج',command=again).grid(row=13, column=2)
    apt.mainloop()
...
def delete_stock(): #OPENS DELETE WINDOW----------------------------------------------------DELETES A PARTICULAR STOCK ITEM
    ...
    flag='d'
    d=Tk()
    d.title("حذف یک محصول از انبار")
    Label(d,text='محصول را برای حذف شدن وارد کنید:').grid(row=0,column=0)
    Label(d,text='',width=30,bg='white').grid(row=0,column=1)
    Label(d,text='محصول').grid(row=2,column=0)
    Label(d,text='تعداد.  تاریخ انقضا.     هزینه                           ').grid(row=2,column=1)
    ren()
    b=Button(d,width=20,text='حذف',command=delt).grid(row=0,column=3)
    b=Button(d,width=20,text='منوی اصلی',command=main_menu).grid(row=5,column=3)
    d.mainloop()
...
def modify():    # window for modification-----------------------------------------------------------------------MODIFY
    ...
    st=Tk()
    st.title('MODIFY')
    Label(st, text='-'*48+' تغییر دیتابیس '+'-'*48).grid(row=0, column=0,columnspan=6)
    def onvsb(*args):
        name_.yview(*args)
    def onmousewheel():
    ...
    Label(st, text='نام محصول را وارد کنید: ').grid(row=1, column=0)
    Label(st, text='مقدار تغییر داده شده را وارد کنید: ').grid(row=2, column=0)
    att=Spinbox(st, values=col_n)
    att.grid(row=2, column=1)
    up=Entry(st)
    up.grid(row=2, column=2)
    Button(st,width=10,text='ثبت', command=save_mod).grid(row=2, column=4)
    Button(st,width=10,text='راه اندازی مجدد', command=res).grid(row=2, column=5)
    Button(st,width=10,text='نمایش داده', command=show_val).grid(row=1, column=4)
    Label(st, text='-'*120).grid(row=3,column=0,columnspan=6)
    Button(st,width=10,text='منوی اصلی',command=main_menu).grid(row=5,column=5)
    st.mainloop()
...
def save_mod(): #save modified data
    ...
    cur.execute(sql)
    c.commit()
    Label(st, text='آپدیت!').grid(row=5,column=4)
...
def stock():    #add to stock window------------------------------------------------------------------------ADD TO STOCK
    ...
    accept=['']*10
    sto=Tk()
    sto.title('ورودی انبار')
    Label(sto,text='داده محصول جدید انبار را وارد کنید').grid(row=0,column=0,columnspan=2)
    ...
    Button(sto,width=15,text='ثبت',command=submit).grid(row=12,column=1)
    Label(sto,text='-'*165).grid(row=13,column=0,columnspan=7)
    Button(sto,width=15,text='را اندازی مجدد',command=reset).grid(row=12,column=0)
    Button(sto,width=15,text='رفرش کردن انبار',command=ref).grid(row=12,column=4)
    for i in range(1,6):
        Label(sto,text=columns[i]).grid(row=14,column=i-1)
    Label(sto,text='exp   rack   manufacture                      ').grid(row=14,column=5)
    Button(sto,width=10,text='منوی اصلی',command=main_menu).grid(row=12,column=5)
    ref()
    sto.mainloop()
...
def submit(): #for new stock submission
    ...
    print (now-prev)
    top=Tk()
    Label(top,width=20, text='موفق!').pack()
    top.mainloop()
    main_menu()
...
def chk(): #checks if the medicine is already present so that can be modified
    ...
    for i in cur:
        if accept[6].get()==i[6] and i[1]==accept[1].get():
            ...
            c.commit()
            top=Tk()
            Label(top,width=20, text='تغییر!').pack()
            top.mainloop()
            main_menu()
        else:
            submit()
...
def exp_date(): # expiry window open-----------------------------------------------------------------------------EXPIRY
    ...
    c.commit()
    exp=Tk()
    exp.title('بررسی انقضا')
    Label(exp,text='امروز : '+str(now[2])+'/'+str(now[1])+'/'+str(now[0])).grid(row=0, column=0, columnspan=3)
    Label(exp,text='فروش داروهای تاریخ مصرف گذشته ممنوع است').grid(row=1, column=0,columnspan=3)
    Label(exp,text='-'*80).grid(row=2, column=0,columnspan=3)
    s=Spinbox(exp,values=n)
    s.grid(row=3, column=0)
    Button(exp,text='بررسی تاریخ انقضا', command=s_exp).grid(row=3, column=1)
    Label(exp,text='-'*80).grid(row=4, column=0,columnspan=3)
    if flags=='apt1':
        Button(exp,text='منوی اصلی', command=main_cus).grid(row=5, column=2)
    else:
        Button(exp,width=20,text='بررسی انقضای محصول', command=exp_dt).grid(row=5, column=0)
        Button(exp,text='منوی اصلی', command=main_menu).grid(row=5, column=2)
    exp.mainloop()
...
def billing(): # to create bills for customer-------------------------------------------------------------BILLING system
    ...
    flag='st'
    st=Tk()
    st.title('سیستم صورتحساب')
    Label(st,text='-'*48+'سیستم صورتحساب'+'-'*49).grid(row=0,column=0,columnspan=7)
    Label(st,text='نام را وارد کنید: ').grid(row=1,column=0)
    name1=Entry(st)
    name1.grid(row=1, column=1)
    Label(st,text='آدرس را وارد کنید: ').grid(row=2,column=0)
    add=Entry(st)
    add.grid(row=2, column=1)
    Label(st,text="آیدی(درصورتّ وجود)").grid(row=3, column=0)
    vc_id=Entry(st)
    vc_id.grid(row=3, column=1)
    Button(st,text='بررسی مشتری', command=blue).grid(row=4, column=0)
    Label(st,text='-'*115).grid(row=6, column=0,columnspan=7)
    Label(st,text='انتخاب محصول',width=25,relief='ridge').grid(row=7, column=0)
    Label(st,text=' RACK  QTY LEFT     COST          ',width=25,relief='ridge').grid(row=7, column=1)
    Button(st,text='افزودن به صورتحساب',width=15,command=append2bill).grid(row=8, column=6)
    Label(st,text='تعداد',width=20,relief='ridge').grid(row=7, column=5)
    qtys=Entry(st)
    qtys.grid(row=8,column=5)
    refresh()
    Button(st,width=15,text='منوی اصلی', command=main_menu).grid(row=1,column=6)
    Button(st,width=15,text='رفرش کردن انبار', command=refresh).grid(row=3,column=6)
    Button(st,width=15,text='صورتحساب جدید', command=billing).grid(row=4,column=6)
    Button(st,width=15,text='چاپ صورتحساب', command=print_bill).grid(row=5,column=6)
    Button(st,width=15,text='ذخیره صورتحساب', command=make_bill).grid(row=7,column=6)
...
def blue(): # check if valued customer
    ...
    for i in cur:
        if vc_id.get()!='' and int(vc_id.get())==i[2]:
            ...
            Label(st, text='مشتری تایید شده!').grid(row=4, column=1)
            t=1
            break
    c.commit()
...
def make_bill(): # makes bill
    ...
    if t==1:
        ntotal=total*0.8
        m+='کل'+(' '*25)+(' '*(15-len(str(total)))) + str(total)+'\n'
        m+="تخفیف مشتری تایید شده"+ (' '*(20-len(str(total-ntotal))))+'-'+str(total-ntotal)+'\n'
        m+="-----------------------------------------------\n"
        m+='کل'+(' '*25)+(' '*(12-len(str(ntotal)))) +'Rs '+ str(ntotal)+'\n'
        det[3]=str(ntotal)
    else:
        m+='کل'+(' '*25)+(' '*(12-len(str(total)))) +'Rs '+ str(total)+'\n'
        det[3]=str(total)
        
    m+="-----------------------------------------------\n\n"
    m+="امضای واسطه:___________________________\n"
    m+="===============================================\n"
    print (m)
    p=time.localtime()
...
def show_rev(): # opens revenue window-----------------------------------------------------------------------TOTAL REVENUE
    ...
    Label(rev,text='امروز: '+today).grid(row=0,column=0)
    cur.execute('select * from bills')
    for i in cur:
        if i[4]==today:
            total+=float(i[3])
    print (total)
    Label(rev,width=22,text='درآمد کل: Rs '+str(total), bg='black',fg='white').grid(row=1,column=0)
    cx=0
    ...
    for i in cur:
        if i[4]==today:
            cx+=1
            lb1.insert(cx,'شماره صورتحساب.: '+str(i[5])+'    : Rs '+str(i[3]))
    c.commit()
    Button(rev,text='منوی اصلی',command=main_menu).grid(row=15,column=0)
    rev.mainloop()
...
def search():   #search window medicine and symptom details---------------------------------SEARCH MEDICINE RACK & SYMPTOMS
    ...
    st=Tk()
    st.title('جستجو')
    Label(st, text=' جستجو برای محصول ').grid(row=0, column=0,columnspan=3)
    Label(st, text='~'*40).grid(row=1, column=0,columnspan=3)
    Label(st, text='نشانه بیماری').grid(row=3, column=0)
    sym=Spinbox(st,values=symp)
    sym.grid(row=3, column=1)
    Button(st,width=15, text='جستجو', command=search_med).grid(row=3, column=2)
    Label(st, text='-'*70).grid(row=4, column=0,columnspan=3)
    if flags=='apt1':
        Button(st,width=15, text='منوی اصلی', command=main_cus).grid(row=6, column=2)
    else:
        Button(st,width=15, text='منوی اصلی', command=main_menu).grid(row=6, column=2)
    st.mainloop()
...
def val_cus():  #to enter new valued customer-----------------------------------------------------------NEW VALUED CUSTOMER
    ...
    flag='val'
    val=Tk()
    Label(val,text="جزییات مشتری تایید شده را وارد کنید").grid(row=0,column=0,columnspan=3)
    Label(val,text="-"*60).grid(row=1,column=0,columnspan=3)
    Label(val,text="اسم: ").grid(row=2,column=0)
    name_vc=Entry(val)
    name_vc.grid(row=2, column=1)
    Label(val,text="آدرس: ").grid(row=3,column=0)
    add_vc=Entry(val)
    add_vc.grid(row=3, column=1)
    Label(val,text="آیدی: ").grid(row=4,column=0)
    vc_id=Entry(val)
    vc_id.grid(row=4, column=1)
    Button(val,text='ارسال',command=val_get).grid(row=5, column=1)
    Button(val,text='منوی اصلی',command=main_menu).grid(row=5, column=2)
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    Label(root,text='فروشگاه دارو').grid(row=0,column=0,columnspan=5)
    ...
    Label(root, text='نام کاربری').grid(row=3, column=0)
    un=Entry(root,width=10)
    un.grid(row=3, column=1)
    Label(root, text='رمز عبور').grid(row=4, column=0)
    pwd=Entry(root,width=10)
    pwd.grid(row=4, column=1)
    Button(root,width=6,text='ورود',command=check).grid(row=5, column=0)
    Button(root,width=6,text='بستن',command=root.destroy).grid(row=5, column=1)
    root.mainloop()
...
def open_cus(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    apt=Tk()
    apt.title("Kosar Najafi 99221001111050")
    Label(apt, text="فروشگاه دارو").grid(row=0,column=0)
    Label(apt, text='*'*40).grid(row=1,column=0)
    Label(apt, text='*  خوش آمدید  *').grid(row=2,column=0)
    Label(apt, text='-'*40).grid(row=3,column=0)
    Label(apt, text="خدمات مشتری").grid(row=4,column=0)
    Label(apt, text='-'*40).grid(row=5,column=0)
    Button(apt,text='جستجو', width=15, command=search).grid(row=6,column=0)
    Button(apt,text='بررسی انقضا', width=15, command=exp_date).grid(row=7,column=0)
    
    Label(apt, text='-'*40).grid(row=8,column=0)    
    Button(apt,text='خروج',command=again1).grid(row=9, column=0)
    apt.mainloop()
def again1():
    global flags
    apt.destroy()
